...
def main() -> None:
    ...
    print("\n[OK] Checking Installed Apps...")
    app_configs = apps.get_app_configs()
    project_apps = [
        app.name for app in app_configs
        if not app.name.startswith('django.')
        and app.name not in ['rest_framework', 'corsheaders', 'django_filters']
    ]
    for app_name in project_apps:
        print(f"  - {app_name}")

    # Test 5: Verify URL patterns
    print("\n[OK] Checking URL Configuration...")
    ...
    print("  - Key endpoints configured: Yes")

    # Test 6: Check Celery configuration
    print("\n[OK] Checking Celery Configuration...")
